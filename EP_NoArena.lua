...
local arenaInviteDeclineMessage = "This is an automated message, all arena invitations will be declined";
local arenaPetitionDeclineMessage = "This is an automated message, all arena petitions will be declined";
...
function EPNoArena:DeclineArenaInvite()
	...
	-- tell the idiot
	SendChatMessage(arenaInviteDeclineMessage, "WHISPER", nil, arg1);
end;
...
function EPNoArena:DeclineArenaPetition(arenaName, originator, isOriginator)
	...
	if isOriginator == true then
		...
		-- tell the idiot
		SendChatMessage(arenaPetitionDeclineMessage, "WHISPER", nil, arg1);
	else
		-- report to user
...
EPNoArena:SetScript("OnEvent", function(self, event, addon)
...
ChatFrame_AddMessageEventFilter("CHAT_MSG_WHISPER_INFORM", function(_, _, msg, player)
	if msg == arenaInviteDeclineMessage then return true; end; -- filter out the reply whisper
	if msg == arenaPetitionDeclineMessage then return true; end; -- filter out the reply whisper
end);
